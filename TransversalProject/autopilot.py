...
def executeFlightPlan(waypoints_json):
    ...
    wp = waypoints['coordinates'][0]
    originPoint = dronekit.LocationGlobalRelative(float(wp[0]), float(wp[1]), altitude)

    distanceThreshold = 0.25
    for wp in waypoints['coordinates'][1:]:

        destinationPoint = dronekit.LocationGlobalRelative(float(wp[0]), float(wp[1]), altitude)
        ...
        time.sleep(3)
        external_client.publish('autopilotService/cameraService/takePicture', '')
...
def executeFlightPlan2(waypoints_json):
    ...
    while True:
        ...
        if nextwaypoint == len(waypoints['coordinates']):
...
def AutopilotService(connection_mode, operation_mode, external_broker, username, password):
    ...
    print("Waiting....")
    print('---------')
    external_client.subscribe("+/autopilotService/#", 1)
